...
<body class="bg-light p-5">
    <div class="container">
        <div class="card shadow mx-auto" style="max-width: 500px;">
            <div class="card-header bg-primary text-white">選手情報の編集</div>
            <form action="update_member.php" method="POST" class="card-body">
                ...
                <select name="team_id" class="form-select mb-3">
                    <?php foreach($teams as $t): ?>
                        <option value="<?=$t['id']?>" <?=$t['id']==$member['team_id']?'selected':''?>><?=$t['team_name']?></option>
                    <?php endforeach; ?>
                </select>

                <label class="small text-muted">選手名</label>
                ...
                <input type="number" name="back_number" class="form-control mb-4" value="<?=$member['back_number']?>" required>

                <div class="d-flex justify-content-between">
                    <a href="team_admin.php" class="btn btn-secondary">戻る</a>
